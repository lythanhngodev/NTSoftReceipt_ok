...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    function OnClientDbClick(record) {
        var permiss = NTSoftReceipt.danhmuc.dmquanhuyen.PhanQuyenChucnang();
        arrValue = permiss.value.split(';');
        if (arrValue[0] == "true") {
            alert("User đang sử dụng chỉ được xem dữ liệu, không thực hiện được thao tác sửa!");
        ...
        coThaotac = 2;
        _txtMa.disable();
        _txtMa.value(Grid1.Rows[record].Cells["maHuyenpr"].Value);
        _txtTen.value(Grid1.Rows[record].Cells["tenHuyen"].Value);
        _txtGhiChu.value(Grid1.Rows[record].Cells["ghiChu"].Value);
        _txtTenVT.value(Grid1.Rows[record].Cells["tenVietTat"].Value);
        _cboTinh.value(Grid1.Rows[record].Cells["maTinhpr_sd"].Value);
        if (Grid1.Rows[record].Cells["ngungSD"].Value == "True")
            _chkNTD.checked(true);
        else
            _chkNTD.checked(false);
        Window1.setTitle("Sửa Quận/Huyện");
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <cc1:Grid ID="Grid1" runat="server" FolderStyle="~/App_Themes/Styles/style_7" AllowPaging="true"
        ...
        <ClientSideEvents OnBeforeClientAdd="OnBeforeClientAdd" OnBeforeClientEdit="OnBeforeClientEdit"
            OnBeforeClientDelete="OnBeforeClientDelete" OnClientDblClick="OnClientDbClick" />
        <Columns>
            ...
            <cc1:Column HeaderText="Ghi chú" DataField="ghiChu" Width="130px" Wrap="true">
            </cc1:Column>
